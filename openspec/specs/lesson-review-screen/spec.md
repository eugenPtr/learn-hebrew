...
The system SHALL render a delete button on each row. Tapping delete SHALL remove the item from the local list only — no DB call.
...
The system SHALL allow the user to tap any row to open an edit modal. The modal SHALL display the row's hebrew and english as editable inputs, render with a blurred background overlay, and provide a Save button and an X (close) button. Tapping Save SHALL update the item in local state and close the modal. Tapping X SHALL close without saving.
