...
- [x] 1.1 Create `components/exercises/TranslatingExercise.tsx` with props interface and state structure
- [x] 1.2 Render `exercise.englishSentence` as the prompt
- [x] 1.3 Render speaker button that plays `exercise.audioUrl` on click (no autoplay)
...
- [x] 3.1 Implement `scoreAnswer(input, reference)` — tokenise, strip punctuation, 70% threshold
